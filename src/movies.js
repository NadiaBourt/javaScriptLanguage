...
// console.log(httpPrefix+moviesData.results[2].backdrop_path);
...
{
const arrayImages = moviesData.results.map(movie =>`<li class="movie-item"><img class ="movie-image" 
src = "${httpPrefix}${movie.poster_path}"></li>`);
return arrayImages.join('');
}

function show(index)
{
sectionsElements.forEach(section => section.hidden = true);
sectionsElements[index].hidden = false;
}

function getBestFilm (index)
{
